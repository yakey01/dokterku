@php
    ...
    // Check if we have a specific jaspel formula selected
    if ($dokter_umum_jaspel_id) {
        $jaspeFormula = DokterUmumJaspel::find($dokter_umum_jaspel_id);
    } else {
        // Fallback to current system for backward compatibility
        $currentHour = now()->hour;
        $jenisShift = $shift ?? match(true) {
...
<div class="space-y-3">
        ...
        @if($jaspeFormula)
            <div class="grid grid-cols-1 gap-3 text-sm">
                ...
                <div class="bg-white dark:bg-gray-800 p-3 rounded border-l-4 border-green-400">
                    <p class="font-medium text-green-800 dark:text-green-200 mb-1">🎯 Formula yang Digunakan</p>
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        {{ $jaspeFormula->shift_display }}
                        @if($dokter_umum_jaspel_id)
                            <span class="text-green-600 font-medium">(Formula Terpilih)</span>
                        @else
                            <span class="text-blue-600 font-medium">(Auto-detect berdasarkan waktu)</span>
                        @endif
                    </p>
                </div>
        ...
        @else
            <div class="text-center py-4">
                ...
                <p class="text-sm text-gray-500 mt-1">
                    ...
                    @else
                        Pilih shift dan formula jaspel terlebih dahulu.
                    @endif
    ...
    <div class="text-xs text-gray-500 dark:text-gray-400 space-y-1">
        <p>ℹ️ Perhitungan menggunakan formula yang dipilih atau auto-detect berdasarkan shift.</p>
